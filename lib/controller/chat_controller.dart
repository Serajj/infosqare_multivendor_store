...
import 'package:connectuz_store/data/model/response/message_model.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> getMessages(int offset, NotificationBody notificationBody,
    ...
    if (notificationBody.customerId != null ||
        notificationBody.type == AppConstants.customer ||
        notificationBody.type == AppConstants.user) {
      response = await chatRepo.getMessages(offset, notificationBody.customerId,
          AppConstants.user, conversationID);
    } else if (notificationBody.deliveryManId != null ||
        notificationBody.type == AppConstants.deliveryMan) {
  ...
  Future<Response?> sendMessage(
    ...
    if (notificationBody != null &&
        (notificationBody.customerId != null ||
            notificationBody.type == AppConstants.customer)) {
      response = await chatRepo.sendMessage(message, myImages, conversationId,
          notificationBody.customerId, AppConstants.customer);
    } else if (notificationBody != null &&
        (notificationBody.deliveryManId != null ||
            notificationBody.type == AppConstants.deliveryMan)) {
      response = await chatRepo.sendMessage(message, myImages, conversationId,
          notificationBody.deliveryManId, AppConstants.deliveryMan);
    }
