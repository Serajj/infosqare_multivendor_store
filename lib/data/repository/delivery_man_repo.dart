...
class DeliveryManRepo {
  ...
  Future<Response> addDeliveryMan(DeliveryManModel deliveryMan, String pass,
      XFile? image, List<XFile> identities, String token, bool isAdd) async {
    http.MultipartRequest request = http.MultipartRequest(
        'POST',
        Uri.parse(
          '${AppConstants.baseUrl}${isAdd ? AppConstants.addDmUri : '${AppConstants.updateDmUri}${deliveryMan.id}'}',
        ));
    ...
    if (GetPlatform.isMobile && identities.isNotEmpty) {
      for (var identity in identities) {
        ...
        request.files.add(http.MultipartFile('identity_image[]',
  ...
  Future<Response> deleteDeliveryMan(int? deliveryManID) async {
    return await apiClient.postData(AppConstants.deleteDmUri,
        {'_method': 'delete', 'delivery_man_id': deliveryManID});
  }
