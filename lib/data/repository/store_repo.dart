...
class StoreRepo {
  ...
  Future<Response> getItemList(String offset, String type) async {
    return await apiClient.getData(
        '${AppConstants.itemListUri}?offset=$offset&limit=10&type=$type');
  }
  ...
  Future<Response> addItem(
    ...
    fields.addAll(<String, String>{
      ...
      'tags': tags,
      'organic': item.organicStatus.toString(),
    });
    ...
    if (item.categoryIds!.length > 1) {
      fields.addAll(
          <String, String>{'sub_category_id': item.categoryIds![1].id!});
    }
    if (!isAdd) {
      fields.addAll(<String, String>{
        '_method': 'put',
  ...
  Future<Response> deleteItem(int? itemID) async {
    return await apiClient
        .deleteData('${AppConstants.deleteItemUri}?id=$itemID');
  }

  Future<Response> getStoreReviewList(int? storeID) async {
    return await apiClient
        .getData('${AppConstants.vendorReviewUri}?store_id=$storeID');
  }
  ...
  Future<Response> updateOrganicProductStatus(
      int? productID, int status) async {
    return await apiClient.getData(
